...
QModelIndex customRemove( NifModel * nif, const QModelIndex & index )
{
...
find out what x is
Save as +enter x times (duplicate file)

Copy the NIF into another file x times
or duplicate original
open file 1 (called name - Copy (1) )
delete other branches (besides texture x branch)
Save as "name - texture name 1"
open file 2
...
//custom
    ...
    QList<qint32> cast2( NifModel * nif )
    {
        QList<qint32> textures;
        QModelIndex first = nif->getHeader();
        QList<qint32> blocks; //blocks is the block u select + all children of it, block 0 is the root of tree
        populateBlocks( blocks, nif, nif->getBlockNumber( first ) );

        QMap<qint32, qint32> blockMap;
        for ( int b = 0; b < blocks.count(); b++ )
            blockMap.insert( blocks[b], b ); //b is the block and also the value also index, blocks[b] is the key contains num of block (index)

        QList<qint32> textures2;
        //populateBlocks( blocks2, nif, nif->getBlockNumber( first ) );

        for ( const auto block : blocks )
        {

            //if ( nif->isNiBlock( block, { "NiTriShape", "NiTriStrips", "BSLODTriShape" } ) )
            if (nif->getBlockType( nif->getBlock((int)block)) == "BSShaderTextureSet") {
                textures.append(block);
            }
            if (nif->getBlockType( nif->getBlock((int)block)).contains("TriShape") && !(nif->getBlockType( nif->getBlock((int)block)).contains("TriShapeData"))) {
                textures2.append(block);
                //nif->getBlockName(nif->getBlock((int)block)).contains()
                //if( nif->getBlockType(nif->getBlock(nif->getParent((int)block))).contains("TriShape") ) {
            }
            if (nif->getBlockType( nif->getBlock((int)block)).contains("TriStrips") && !(nif->getBlockType( nif->getBlock((int)block)).contains("TriShapeData"))) {
                textures2.append(block);
            }
            spRemoveBlock::cast(nif->getBlock((int)0)))
        }
        return textures2;
